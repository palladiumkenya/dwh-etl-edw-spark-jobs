package org.kenyahmis.factpartnernotification;
...
public class LoadFactPartnerNotification {
    private static final Logger logger = LoggerFactory.getLogger(LoadFactPartnerNotification.class);

    public static void main(String[] args) {

        SparkConf conf = new SparkConf();
        conf.setAppName("Load Fact Partner Notification Services");
        SparkSession session = SparkSession.builder()
                .config(conf)
                .getOrCreate();
        RuntimeConfig rtConfig = session.conf();

        LoadFactPartnerNotification loadFactPartnerNotification = new LoadFactPartnerNotification();
        final String loadPartnerNotificationSourceQueryFileName = "PartnerNotificationSource.sql";
        String loadPartnerNotificationSourceQuery = loadFactPartnerNotification.loadQuery(loadPartnerNotificationSourceQueryFileName);

        Dataset<Row> partnerNotificationSourceDataDf = session.read()
                .format("jdbc")
                .option("url", rtConfig.get("spark.ods.url"))
                .option("driver", rtConfig.get("spark.ods.driver"))
                .option("user", rtConfig.get("spark.ods.user"))
                .option("password", rtConfig.get("spark.ods.password"))
                .option("query", loadPartnerNotificationSourceQuery)
                .load();

        // Hash CCC Number & Partner PK
        partnerNotificationSourceDataDf = partnerNotificationSourceDataDf.withColumn("CCCNumber",
                upper(sha2(col("CCCNumber").cast(DataTypes.StringType), 256)))
        .withColumn("PartnerPatientPk", upper(sha2(col("PartnerPatientPk").cast(DataTypes.StringType), 256)));
        partnerNotificationSourceDataDf.createOrReplaceTempView("source_data");
        partnerNotificationSourceDataDf.persist(StorageLevel.DISK_ONLY());
        ...
        String factPartnerNotificationQuery= loadFactPartnerNotification.loadQuery("LoadFactPartnerNotification.sql");
    ...
    private String loadQuery(String fileName) {
        String query;
        InputStream inputStream = LoadFactPartnerNotification.class.getClassLoader().getResourceAsStream(fileName);
        if (inputStream == null) {
