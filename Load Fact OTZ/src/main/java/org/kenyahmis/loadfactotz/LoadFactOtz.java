package org.kenyahmis.loadfactotz;
...
public class LoadFactOtz {
    ...
    public static void main(String[] args) {
        ...
        final String loadOTZLastEncounterCombinedQueryFileName = "LoadOTZAndLastEncounterCombined.sql";
        LoadFactOtz loadFactOtz = new LoadFactOtz();
        String loadOtzLastEncounterCombinedQuery = loadFactOtz.loadQuery(loadOTZLastEncounterCombinedQueryFileName);
        if (loadOtzLastEncounterCombinedQuery == null) {
            return;
        }

        Dataset<Row> otzAndLastEncounterDataFrame = session.read()
                .format("jdbc")
                .option("url", rtConfig.get("spark.ods.url"))
                .option("driver", rtConfig.get("spark.ods.driver"))
                .option("user", rtConfig.get("spark.ods.user"))
    ...
    private String loadQuery(String fileName) {
        String query;
        InputStream inputStream = LoadFactOtz.class.getClassLoader().getResourceAsStream(fileName);
        if (inputStream == null) {
