package org.kenyahmis.loadfactdefaultertracing;
...
public class LoadFactDefaulterTracing {
    private static final Logger logger = LoggerFactory.getLogger(LoadFactDefaulterTracing.class);
    public static void main(String[] args) {

        SparkConf conf = new SparkConf();
        conf.setAppName("Load Defaulter Tracing Fact");
        ...
        final String loadVisitDataQueryFileName = "LoadVisitData.sql";
        LoadFactDefaulterTracing loadFactDefaulterTracing = new LoadFactDefaulterTracing();
        String loadVisitDataQuery = loadFactDefaulterTracing.loadQuery(loadVisitDataQueryFileName);

        Dataset<Row> visitDataFrame = session.read()
                .format("jdbc")
                .option("url", rtConfig.get("spark.ods.url"))
                .option("driver", rtConfig.get("spark.ods.driver"))
                .option("user", rtConfig.get("spark.ods.user"))
                .option("password", rtConfig.get("spark.ods.password"))
                .option("query", loadVisitDataQuery)
                .load();
        visitDataFrame.createOrReplaceTempView("visits_data");
        visitDataFrame.persist(StorageLevel.DISK_ONLY());
        visitDataFrame.printSchema();
        ...
        factDefaulterTracingDf
                ...
                .option("user", rtConfig.get("spark.edw.user"))
                .option("password", rtConfig.get("spark.edw.password"))
                .option("dbtable", "dbo.FactDefaulterTracing")
    ...
    private String loadQuery(String fileName) {
        String query;
        InputStream inputStream = LoadFactDefaulterTracing.class.getClassLoader().getResourceAsStream(fileName);
        if (inputStream == null) {
