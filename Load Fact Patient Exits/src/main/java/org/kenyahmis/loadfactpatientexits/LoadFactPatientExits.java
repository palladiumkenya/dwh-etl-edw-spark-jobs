package org.kenyahmis.loadfactpatientexits;
...
public class LoadFactPatientExits {
    private static final Logger logger = LoggerFactory.getLogger(LoadFactPatientExits.class);
    public static void main(String[] args) {
        SparkConf conf = new SparkConf();
        conf.setAppName("Load Patient Exits Fact");
        ...
        final String loadExitsQueryFileName = "LoadExits.sql";
        LoadFactPatientExits loadPatientExits = new LoadFactPatientExits();
        String loadPatientQuery = loadPatientExits.loadQuery(loadExitsQueryFileName);
        ...
        Dataset<Row> patientDataFrame = session.read()
                ...
                .load();
        patientDataFrame.createOrReplaceTempView("Exits");
...
//        factExitsDf = factExitsDf.withColumn("FactKey", monotonically_increasing_id().plus(1));
    ...
    private String loadQuery(String fileName) {
        String query;
        InputStream inputStream = LoadFactPatientExits.class.getClassLoader().getResourceAsStream(fileName);
        if (inputStream == null) {
