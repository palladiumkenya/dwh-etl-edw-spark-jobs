package org.kenyahmis.loadhtsfacteligibility;
...
public class LoadFactHtsEligibility {
    private static final Logger logger = LoggerFactory.getLogger(LoadFactHtsEligibility.class);

    public static void main(String[] args) {
        SparkConf conf = new SparkConf();
        conf.setAppName("Load Fact Hts Eligibility");
        SparkSession session = SparkSession.builder()
                .config(conf)
                .getOrCreate();
        RuntimeConfig rtConfig = session.conf();

        LoadFactHtsEligibility loadFactHtsEligibility = new LoadFactHtsEligibility();
        final String eligibilitySourceDataQueryFileName = "EligibilitySourceData.sql";
        String eligibilitySourceQuery = loadFactHtsEligibility.loadQuery(eligibilitySourceDataQueryFileName);

        Dataset<Row> clientEligibilitySourceDataDf = session.read()
                .format("jdbc")
                .option("url", rtConfig.get("spark.ods.url"))
                .option("driver", rtConfig.get("spark.ods.driver"))
                .option("user", rtConfig.get("spark.ods.user"))
                .option("password", rtConfig.get("spark.ods.password"))
                .option("query", eligibilitySourceQuery)
                .load();

        // Hash CCC Number
        clientEligibilitySourceDataDf = clientEligibilitySourceDataDf.withColumn("CCCNumber",
                upper(sha2(col("CCCNumber").cast(DataTypes.StringType), 256)));
        clientEligibilitySourceDataDf.createOrReplaceTempView("source_data");
        clientEligibilitySourceDataDf.persist(StorageLevel.DISK_ONLY());
        ...
        String factClientEligibility = loadFactHtsEligibility.loadQuery("LoadFactHtsEligibility.sql");
        Dataset<Row> factHtsEligibilityDf = session.sql(factClientEligibility);

        // Add FactKey Column
        WindowSpec window = Window.orderBy("EncounterId");
    ...
    private String loadQuery(String fileName) {
        String query;
        InputStream inputStream = LoadFactHtsEligibility.class.getClassLoader().getResourceAsStream(fileName);
        if (inputStream == null) {
